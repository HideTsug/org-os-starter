...
- [ ] **参照先が読者の言語の文書を指している。** `docs/ja/` 内の参照は、日本語版が存在する限りそちらを指す。それ以外の場所では英語正本を指す — `AGENTS.md` の読み順の1件目（日本語の `README.md` ではなく `README.en.md`）と、**両方**の README の読み順表・ファイルマップのパス列（両者は同じ英語正本のパスを行単位で一致させる。読み順表のREADME自身の行だけが例外で、各READMEが自分自身のファイル名を書く）を含む。日本語ミラーは補足リンクとして併記してよいが、行のパスそのものにはしない。規約は [CLAUDE.md](CLAUDE.md)「上流リポジトリ専用の規約」にある。本節末尾「検査」の1つ目のスニペットは `docs/ja/` の外へ出るリンクをすべて列挙する。残る各件は意図した例外（`English version:` の冒頭リンク・日本語版である `README.md`・`LICENSE` のように日本語版が存在しないファイル）でなければならない。`python3` が無い環境では、同スニペットを参照実装として扱い、node・ripgrep＋シェルループ・変更した `docs/ja/` 配下ファイルの直接確認などで同等の出力を作る
...
どちらのスニペットもリポジトリルートで実行する。1つ目は「参照先が読者の言語」の項目に対応し、`docs/ja/` の外へ出るリンクをすべて列挙する。2つ目は「相対リンクがすべて解決」の項目に対応し、`scripts/validate.py` のリンク検査部分を切り出したもので、`python3` が無い環境が他のツールで同等の出力を作るための参照実装。
...
`python3 scripts/validate.py` が違反を報告したとき、違反の各行は5つのタイプ名のどれかで始まる。タイプ名を次の表で引き、意味と直し方を確認する:

| 違反タイプ | 意味 | 直し方 |
|---|---|---|
| `relative-link` | 相対 Markdown リンクの参照先が存在しない（ディレクトリや `LICENSE` など Markdown 以外の参照先も対象） | パスを直すか参照先を復元する。移動・改名をしたときは、旧パスを指していたリンクを両言語とも更新する |
| `wikilink` | `[[wikilink]]` がリポジトリ内の Markdown ファイルに解決しない（basename で照合。ターゲットに `/` を含む場合はリポジトリパスで照合） | ターゲットを実在ファイルの basename またはパスに正確に一致させるか、欠けているファイルを復元する |
| `status` | frontmatter の `status` 値が `draft` / `proposed` / `agreed` のどれでもない | 3値のどれかにする。上流 PR でテンプレートの `status` を昇格させない — 上のチェックリストの frontmatter 項目を参照 |
| `placeholder` | コードブロック・コードスパンの外に裸の `(TODO)` `(TBD)` `(FIXME)` がある | `(要・代表)` のような責任者フラグに置き換える — [CLAUDE.md](CLAUDE.md)「構造ルール」参照 |
| `file-map` | `README.md` または `README.en.md` の全ファイルマップが実ツリーとどちらかの方向で食い違う、またはマップブロックが見つからない | 追加・移動・改名を**両** README のファイルマップへ同じく反映する |
...
リリースはメンテナが切る。版数・タグ・リリースノートは `CHANGELOG.md` と両 README の更新戦略が導入組織に案内している参照先なので、以下の4手順で1つの操作とする。文書だけ直して止めると、告知されているのに取得できない版と、解決しない比較リンクが残る。

1. `CHANGELOG.md` と `docs/ja/CHANGELOG.md` の `## [Unreleased]` 配下の項目を新しい `## [X.Y.Z] - YYYY-MM-DD` 節へ移す。あわせて両ファイルの `## [Unreleased]` 直下の一行の基準版を新しい版へ更新し、末尾の比較リンクも更新する
2. `README.md` と `README.en.md` 冒頭の現行バージョン行を更新する
3. 手順1・2 の Pull Request をマージする。古い `## [Unreleased]` 節から分岐した他の PR は、マージ前に新しい `main` へリベースしたうえで、CHANGELOG の追記行を手で新しい `## [Unreleased]` 配下へ移す。リベースは同じ見出しへ再適用するだけで、その見出しは切ったばかりの節の中にあるため、放置すると公開済みリリースを遡及変更し、次のリリースからその変更が抜け落ちる
4. マージ後のコミットにタグを打ち、手順1の節を本文として GitHub Release を発行する: `gh release create vX.Y.Z --target <マージコミットの SHA> --title "vX.Y.Z" --notes-file <file>`。`main` ではなくマージが生成した SHA を渡す（手順3以降に入った別の PR をタグに巻き込まないため）

### 版数の付け方

このリポジトリは複製して分岐するテンプレートであり、複製側は依存解決ではなく両 README の更新戦略で上流を取り込む。したがって公開パスは API 契約ではなく、版数は「上流の版を取り込むとき導入組織が何をやり直すことになるか」として読む。

- **MAJOR** — 5層構造または導入フローの破壊的変更
- **MINOR** — 機能追加
- **PATCH** — 修正

今後、公開パスの削除・改名を含むリリースは MAJOR とする。v1.0.0 → v1.1.0 は英語 canon 化に伴う移行期の例外で、非互換は `CHANGELOG.md` の `[1.1.0]` 節の旧→新パス対応表で告知する。

## スコープ外の注記

- このリポジトリは GitHub Actions のワークフローを同梱していない。ただし、メンテナの Release 運用専用の `release-assets.yml` は例外で、利用者の派生 repo では Release を publish しない限り動かない。CI・issue テンプレート・`.github/` 配下の追加はサプライチェーンに関わるメンテナの判断事項。ワークフローを PR で送るのではなく issue を立てる
