...
多くのエージェントは repo を開いた時点で本ファイルを自動で読み込む。あなたのタスクが **このリポジトリのファイルを変更すること**（typo 修正・訳の修正・テンプレート改善など）なら、ここで止まって [`CLAUDE.md`](CLAUDE.md)（編集時の運用規範）と [`CONTRIBUTING.md`](CONTRIBUTING.md)（何を上流に還元するか・PR の出し方）を読む。以下の導入の流れはあなたのタスクではない（導入組織のセットアップを案内するための手順）。

このリポジトリは Markdown のみで構成される。ビルド・依存インストール・テストスイートは存在しないので探さない。変更後に満たすべきゲートは次の4つ:

1. **相対リンクがすべて解決する。** ファイルを追加・移動・改名した場合は、同じ変更の中で `README.md` と `README.en.md` のファイルマップ・読み順表も更新する。実行可能な検査は [`CONTRIBUTING.md`](CONTRIBUTING.md) にある
2. **英語 canon と日本語ミラーを同時に更新する。** 英語文書が canon で `docs/ja/` はその日本語ミラー。片側だけの編集はリポジトリを不整合にする
3. **サンプルは架空データのみ。** 実在の顧客・取引先・人事情報を含めない。鍵・トークン・シークレットも同様
4. **Layer 3〜5 のディレクトリ・空ディレクトリ・プレースホルダのみのファイルを作らない。** これらが無いのは設計原則であり、記述漏れではない
