...
- **`docs/ja/` の外で参照先をどちらの言語にするか。** 英語正本（root の `AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md`・`README.en.md`・`docs/ja/` を除く `docs/**`）の参照はすべて英語正本を指す。`AGENTS.md` の読み順の1件目も日本語の `README.md` ではなく `README.en.md` とする。`README.md` は日本語の入口だが `docs/ja/` のミラーではない — 読み順表とファイルマップはリポジトリの実ツリーを表す構造なので、この2つの構造だけは `README.en.md` と同じ英語正本のパスを行単位で一致させる。日本語ミラーは、説明列やこの2構造の外の本文で日本語読者向けの補足リンクとして併記してよいが、行のパスそのものにはしない
