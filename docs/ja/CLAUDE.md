...
- 英語版が正本で、`docs/ja/` はその日本語ミラー。`docs/ja/` 内の参照（`[[wikilink]]` のリンク先・フロントマターの値・本文中に書くファイル名）は、日本語版が存在する限り日本語版を指す。日本語版が無い場合のみ英語正本を指す。文書冒頭の `English version:` / `Japanese version:` の相互リンクのみ例外
